...
function matchPattern(pattern, words, pi, wi){
	if (pi == pattern.length && wi == words.length){
		return [];
	}
	...
	if (pattern[pi] == words[wi]){
		return matchPattern(pattern, words, pi+1, wi+1)
	}
}
